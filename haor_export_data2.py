...
"""
==============================================================================
HAOR FLOOD ANALYSIS — DATA EXPORT SCRIPT — version 2 (June, revised)
==============================================================================
Run this as a batch job on your HPC server. It exports all analysis layers
from GEE to Google Drive as GeoTIFFs, and computes time series saved as CSV.

Usage:
    python haor_export_data2.py

Output directory: /work/a06/wasif/haor_flood_analysis/

WHAT CHANGED IN THIS REVISION (and why)
  [NEW]     Section A2 — per-date flood STACK for the 5-haor cluster, exported
            as ONE multiband GeoTIFF (one band per S1 pass; 1=flood, 0=dry,
            nodata=unobserved). This is the input the notebooks need for the
            per-haor hydrographs and the dynamic-connectivity analysis. It did
            not exist before — onset was the only per-date product, and it was
            already reduced. New Drive folder: 'haor_flood_masks'.
  [NEW]     Section A  — 'last_wet_date' (recession/offset map) for the broader
            region. The supervisor asked for onset/peak/offset maps; we had only
            onset (first_wet_date). Offset is the same date band reduced with max.
  [UPDATED] Section B  — screen CoCoAH polygons by long-term JRC occurrence to
            drop near-permanent water bodies (keep seasonal haors). This is the
            supervisor's screening request; it changes cocoah_haor_boundaries.
  [UPDATED] Section G  — total-area time series now reports flooded / OBSERVED
            (a fraction), not raw km². This removes the saw-tooth oscillation,
            which was a satellite-coverage artifact, not real drying.
  [UPDATED] Credentials now read from environment variables (GEE_REFRESH_TOKEN,
            GEE_CLIENT_ID, GEE_CLIENT_SECRET) instead of being hardcoded — so
            this file never carries secrets and runs on a headless server.

DELETE BEFORE RE-RUNNING (so new files land cleanly)
  Drive 'haor_flood_analysis':  old cocoah_haor_boundaries.* (shp/shx/dbf/prj)
  Drive 'haor_flood_masks':     create it (new); on re-runs delete old stack
  Local: total_flood_area_timeseries_2025.csv   (schema changed: new columns)
  Local: per_haor_area_timeseries_2025.csv      (haor set changed by screening)
==============================================================================
"""
import sys
import os
...
MASKS_GDRIVE_FOLDER = "haor_flood_masks"   # [NEW] holds the per-date flood stack
...
BBOX = [89.8, 23.5, 92.8, 25.5]                  # broad region (W,S,E,N)
CLUSTER_BBOX = [90.85, 24.85, 91.50, 25.25]      # [NEW] the 5-haor Tanguar cluster (W,S,E,N)
...
# --- Initialize GEE ---
# [UPDATED] credentials come from environment variables, never hardcoded.
# On an interactive machine the project init below uses your cached auth;
# on a headless server, export GEE_REFRESH_TOKEN / GEE_CLIENT_ID /
# GEE_CLIENT_SECRET first (same pattern as step1_export_reference_data.py).
try:
    ...
    try:
        ...
        ee.Initialize(credentials=credentials,
                      project=os.environ.get("GEE_PROJECT", "79803231644"))
        print("✓ GEE initialized from environment credentials")
    except Exception as e:
        print(f"✗ GEE initialization failed: {e}")
        print("  Set GEE_REFRESH_TOKEN / GEE_CLIENT_ID / GEE_CLIENT_SECRET, "
              "or run `earthengine authenticate` on an interactive machine.")
        sys.exit(1)

roi = ee.Geometry.Rectangle(BBOX)

# ===========================================================================
# SECTION A: FLOOD MASKS, FIRST-WET (onset) AND LAST-WET (offset)
# ===========================================================================
...
def compute_flood_mask(image):
    # Z-score vs each pixel's own dry-season normal (DeVries 2020). Pixels the
    # satellite did not observe stay MASKED here — that is the unobserved class,
    # and it is carried through as nodata into every product below.
...
# Date band = days since Jan 1, stamped only where the pixel is wet.
...
# ONSET = earliest wet day; OFFSET = latest wet day (recession end).
first_wet_date = flood_masks.map(add_date_band).min().rename("first_wet_date")
last_wet_date  = flood_masks.map(add_date_band).max().rename("last_wet_date")   # [NEW] offset map
...
print("✓ Flood layers computed (onset + offset)")
...
# [UPDATED] Occurrence screening — drop near-permanent water bodies.
# WHY: the detector over-counts by catching permanent ponds. We measure each
# polygon's long-term mean JRC occurrence and keep only the seasonal ones
# (mean occurrence < 90%). This is the supervisor's screening request and turns
# the over-counting weakness into a defensible filter. It CHANGES the exported
# cocoah_haor_boundaries — delete the old one in Drive first.
def add_mean_occurrence(feature):
    occ = water_occurrence.reduceRegion(
        reducer=ee.Reducer.mean(), geometry=feature.geometry(),
        scale=100, maxPixels=1e9).get("occurrence")
    return feature.set("mean_occurrence", occ)

haor_vectors = haor_vectors.map(add_mean_occurrence).filter(
    ee.Filter.lt("mean_occurrence", 90))
...
print(f"✓ CoCoAH (after occurrence screen): {n_haors} haors detected")
...
exports = {
    ...
    "last_wet_date": (last_wet_date.clip(roi).float(), EXPORT_SCALE_COARSE),   # [NEW] offset map
...
# Export CoCoAH haor boundaries as shapefile (now occurrence-screened — delete
# the old cocoah_haor_boundaries.* in Drive before this lands)
...
print(f"  Plus the flood stack in → '{MASKS_GDRIVE_FOLDER}' folder")
...
# ===========================================================================
# NOTE: this is the per-haor flooded area for the AUTO-DETECTED CoCoAH haors,
# indexed by haor_idx. Because the occurrence screen in Section B changed the
# haor set, the old per_haor_area_timeseries_2025.csv has stale indices — DELETE
# it before re-running so the resume logic does not skip the wrong haors.
...
# ===========================================================================
# SECTION G: TOTAL AREA TIME SERIES (coverage-corrected)  [UPDATED]
# ===========================================================================
# WHY THE CHANGE: the old curve reported raw flooded km², which DROPS whenever
...
for i, date_str in enumerate(composite_dates):
    try:
        ...
        fa = ee.Number(flood_c.multiply(px).reduceRegion(
...
with open(csv_total, "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=["date", "flood_km2", "observed_km2", "flooded_fraction"])
    writer.writeheader()
